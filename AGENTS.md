...
# (only set this for components that need post-install auth, e.g. gh/Codex):
...
- **Scope**: register with `system` for steps that run as root (most apt-based installs), `user` for steps that need to run as `$USERNAME` (nvm, bun, Codex). For user-scope work inside an install fn, use:
  ```bash
