...
| `bun`, `pnpm`, `claude`, `opencode`, `vercel`, `neon` | npm registry, global installs (`bun`, `pnpm`, `@anthropic-ai/claude-code`, `opencode-ai`, `vercel`, `neonctl`) |
...
`bun`, `pnpm`, `claude`, `opencode`, `vercel` and `neon` are registered `system`
scope but install through `npm -g`, so all six have a hard ordering dependency on
`node` appearing earlier in the registry. The group order holds that for free:
`node` sits in `languages`, ahead of `packaging`, `agents` and `cloud`.
